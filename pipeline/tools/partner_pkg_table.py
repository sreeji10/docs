...
A **provider** is a company or platform that hosts AI models and exposes them through an API (e.g., OpenAI, Anthropic, Google). Each provider has a dedicated `langchain-provider` package that implements LangChain's standard interface, giving you a single API for any model. Install a provider package, pick a model name, and the same code works across all providers.
