...
- Added the ability for users to find account labels for first-class providers, enhancing account management.
- Fixed an issue where switching alerts did not switch the corresponding chart, improving data visualization.
- Fixed a KeyError for experiments with attachments, ensuring stability in experiment management.
- Added a dismiss button to the agentify banner in the Fleet interface for improved usability.
- Enhanced responsiveness in the run details header and trace comparisons for a better user experience.
- Added a truncate property to feedback chips list, making it responsive to container width.
- Fixed a race condition in archive check within the backend, enhancing system reliability.
- Patched multiple medium and high-security alerts, strengthening system security.
- Updated model cards in the frontend for better model information display.
- Improved prompt picker loading when opening the playground from the experiments table.
- Added new onboarding operations to audit logs for improved change tracking.
- Introduced Ashby integration in the Agent Builder, expanding toolset support.
- Enhanced loading speed by parallelizing graph loading and eliminating redundant MCP fetches in the Agent Builder.
- Added a Dynatrace webhook integration for alerts, expanding monitoring options.
- Added feedback banner for experiment sidebar redesign, incorporating user feedback.
- Added example attachments to experiment detail panes for richer experiment context.
- Integrated the arcade with real OAuth flow in the Agent Builder, enhancing authentication processes.
- Improved the fetching of full run data in the new experiment detail pane, ensuring complete data visibility.
...
- Added ability for users to find account labels for first-class providers, enhancing navigation.
- Fixed issue where switching alerts did not update the chart display.
- Fixed a KeyError for experiments with attachments, improving stability.
- Added a dismiss button to the agentify banner for better user control.
- Improved responsiveness in run details header and trace comparison.
- Enhanced UI with a truncate property for feedback chips list, responding to container width.
- Fixed subpixel bleed issue on body rows in the repetition summary table.
- Fixed a race condition in the archive check of aq runs, enhancing reliability.
- Patched multiple security alerts, enhancing overall security.
- Renamed "Metadata" to "Attributes" in the comparison detail pane for clarity.
- Enabled toggle panel size button in Repetition Detail Pane for a customizable interface.
- Updated model cards for better information display.
- Fixed load of prompt picker when opening playground from the experiments table.
- Updated environment promotion buttons to properly disable without tag permissions.
- Onboarded more create, update, delete operations in audit logs for better tracking.
- Added Ashby integration migration in the Agent Builder for new functionality.
- Introduced LLM gateway proof-of-concept, paving the way for new model integrations.
- Parallelized graph loading in Agent Builder for improved performance.
- Added integration with Dynatrace via webhook for enhanced alerting functions.
- Improved experiment detail panes with example attachments, enriching data context.
- Integrated Arcade with real OAuth flow in Agent Builder for seamless authentication.
- Fixed host revisions table to prevent loading flash during revalidation.
- Fixed authentication middleware to include request context in error logs, aiding in troubleshooting.
- Improved prompt editing experience in the evaluator.
...
- Fixed issue where switching alerts did not switch the chart.
- Fixed KeyError for experiments with attachments.
- Added a dismiss button to the agentify banner in the UI.
- Improved responsiveness in run details header and trace comparisons.
- Fixed subpixel bleed on body rows in repetition summary table.
- Fixed a race condition in the smith-backend run archive check.
- Fixed nine medium security alerts, including four high security ones.
- Renamed 'Metadata' to 'Attributes' in comparison detail pane.
- Implemented new toggle panel size button in RepetitionDetailPane.
- Updated model cards in the frontend.
- Fixed load prompt picker when opening playground from experiments table.
- Enabled granular usage rollup cron for self-hosted instances.
- Enhanced audit logs with more CUD operations.
- Added integration migration for the Ashby tool in the Agent Builder.
- Improved performance by parallelizing graph loading and eliminating redundant MCP fetches in agent-builder.
- Added auto expand/collapse keys functionality.
- Added blue hover state to trace comparison divider.
- Avoided proxy MITM races during sandbox startup.
- Fixed granular usage chart bar height calculation.
- Added Dynatrace webhook integration for alerts.
- Added feedback banner for experiment sidebar redesign.
- Added attachments to experiment detail panes in the frontend.
- Wired Arcade integration to real OAuth flow in the agent-builder.
- Prevented loading flash in host revisions table during revalidation.
- Fixed imports in host backend for truststore.
- Improved handling of authentication middleware error logging in agent-builder.
- Fixed editing prompt in evaluator.
- Fetch full run data in new experiment detail pane to improve data loading experience.
...
- Fixed ABAC permission checks path prefix for improved self-hosted security.
- Improved handling of `DotDict` in agent-builder for better decryption of passthrough headers.
- Added network allow denylist for enhanced infrastructure security.
- Added access control UI for sandbox proxy to improve user interface management.
- Improved handling of GCS workload identity in storage with a health check feature.
- Added timeout and reduced concurrency for scheduled insights jobs, enhancing performance.
- Reduced number of preload chunks during initial frontend load to improve loading speeds.
- Cached `/info` and `/auth/v1/user` responses in localStorage for improved performance.
- Enabled JWT generation for auth-proxy in playground service.
- Recreation of feedbacks index to fix backend issues.
- Implemented repository ownership gating for workspace skill editing in agent-builder.
- Generated evaluator JWTs for LLM-as-judge evaluations in backend.
- Always displayed creator name on agent workspace card for better transparency.
- Added static TTL expiry for sandbox claims for better claim management.
- Enabled Slack channels for personal agents in the agent builder.
- Revised inbox tab order for improved user experience.
- Introduced comparison view details pane for more comprehensive data analysis.
- Fixed UI issues including checkbox replacements in various sections for improved user interface consistency.
- Added support for Google IAP session refresh for self-hosted deployments.
- Improved handling of tool registry in playground with delete functionality.
- Added prompt tag environments for enhanced backend configuration.
- Added secret reference support for specifying environment variables in host-backend.
- Updated components and fixed issues with self-hosted deployments.
...
- Fixed path prefix issue for ABAC permission checks to improve self-hosted deployments.
- Enhanced agent builder to handle DotDict structures in decrypt passthrough headers.
- Supported dark mode for fleet logos by using currentColor.
- Updated Slack reauthentication copy with a link to the integrations page in the agent builder.
- Added network allowlist and denylist functionality.
- Introduced UI for access control for sandbox proxy.
- Fixed CopyObject for GCS workload identity and added a copy health check.
- Reduced concurrency and added timeout for scheduled insights jobs to improve performance.
- Reduced the number of preload chunks during initial load to enhance frontend performance.
- Cached `/info` and `/auth/v1/user` responses in localStorage for faster loading.
- Integrated JWT generation for auth-proxy in playground service.
- Improved agent builder: skill editing gated by repository ownership.
- Enabled LLM-as-judge evaluation by generating evaluator JWTs.
- Always showed creator names on agent workspace cards.
- Added repetition support to run details panel in experiments.
- Made the apply button size in the playground consistent with other buttons.
- Added settings for `aud` in the auth proxy UI.
- Fixed LLM auth proxy JWT signing key loading from environment variables.
- Supported Google IAP session refresh for self-hosted deployments.
- Enabled Slack channels for personal agent builders.
- Added service key authentication to runs, sessions, and sandbox endpoints.
- Reordered inbox tabs for better user navigation.
- Added TTL fields to the sandbox UI for clearer management.
- Replaced checkboxes in various UI parts for enhanced interaction consistency.
- Supported internal CLIO_LANGGRAPH_URL for better integration.
- Added support for specifying environment variables from secret reference in host-backend.
- Updated langgraph-api version to fix self-hosted 403 errors.
...
- Fixed path prefix for ABAC permission checks in self-hosted environments.
- Added handling for DotDict in decrypt_passthrough_headers within the Agent Builder.
- Added network allow denylist feature for improved network security.
- Added a UI for access control in sandbox proxy.
- Fixed CopyObject for GCS workload identity and added a copy health check.
- Reduced concurrency and added timeout for scheduled insights jobs to improve performance.
- Reduced number of preload chunks during initial frontend load for faster loading.
- Supported OpenAI chat completion integration.
- Cached /info and /auth/v1/user responses in localStorage for improved frontend performance.
- Added JWT generation for auth-proxy in the playground service.
- Recreated feedbacks index to improve data integrity.
- Gate workspace skill editing in Agent Builder by repo ownership.
- Added features for Slack channels in personal agents within the Agent Builder.
- Added static TTL expiry for sandbox claims.
- Enabled support for Google IAP session refresh in self-hosted environments.
- Supported internal CLIO_LANGGRAPH_URL configuration.
- Added support for specifying environment variables from secret references in host-backend deployments.
- Updated langgraph-api to fix a 403 error in self-hosted instances.
...
- Fixed ABAC permission checks for self-hosted deployments.
- Fixed handling of DotDict in decrypt_passthrough_headers within the Agent Builder.
- Improved dark mode support for fleet logo by using currentColor.
- Added network allow denylist feature for enhanced security.
- Added access control UI for sandbox proxy in the frontend.
- Fixed GCS workload identity and added a copy health check in storage.
- Reduced concurrency and added timeout for scheduled insights jobs to enhance performance.
- Added caching of `/info` and `/auth/v1/user` responses in localStorage for improved frontend performance.
- Implemented JWT generation for auth-proxy in playground service for better security.
- Improved run status icon contrast in light mode on the frontend.
- Added static TTL expiry and TTL fields to sandbox claims for better session management.
- Enhanced Run Details panel with repetition support in experiments.
- Added support for Google IAP session refresh in self-hosted deployments.
- Fixed trace count dashboard issue when grouped by metadata key.
- Fixed the design of the run detail dropdown to improve user experience.
- Added feature to support RDS DB instance for Kubernetes platform.
- Added service key authentication to runs, sessions, and sandbox endpoints for enhanced security.
- Introduced foundation types and set-difference deduplication in messages.
- Enabled Slack channels for personal agents in Agent Builder.
- Improved error handling for non-UTF8 strings in queuing runs and V2 ingest.
- Updated Slack custom-app assistants to run as runner in Fleet.
- Added comparison view details, metrics table, and feedback to enhance the UI.
- Added support for specifying environment variables from secret references in host-backend for better infrastructure management.
- Bumped langgraph-api to fix a self-hosted 403 error.
...
- Added frontend connection to Forge, improving user interaction.
- Enhanced the Agent Builder with a grayed-out identity option in badges for better UI clarity.
- Fixed icon compatibility with Radix tooltips to better display information.
- Made the tracing support email a clickable mailto link for user convenience.
- Updated Forge settings with a connection option to GitHub repositories.
- Fixed removal of stray "Linked" fallback in the UI for cleaner interface.
- Introduced a new endpoint to support offline self-hosted Agent Builder usage.
- Improved the UI by aligning 'no attachments' text to the left within a row.
- Updated the styling for Polly shortcut using kbd styling for consistency.
- Fixed the connect/disconnect button functionality for Slack integrations.
- Redesigned the secrets settings page for a better UX.
- Added migrations for prompt environments to support backend updates.
- Prevented long text from overflowing in DeleteConfirmationModal for better text handling.
- Made the display name on members table inline editable for user flexibility.
- Enabled tenant isolation for price map insertion to enhance multi-tenancy support.
- Fixed frontend issues with tab highlighting to ensure correct navigation feedback.
- Fixed the vacuous connected state on tool badges within the Agent Builder.
- Added authentication support for ABAC and hub repositories.
...
- Added forge frontend and GitHub repository connection to forge settings.
- Improved Agent Builder by showing a grayed-out identity option in AgentTypeBadge trigger.
- Fixed a bug in the frontend to make the tracing support email a mailto link.
- Fixed UI issue by removing stray "Linked" fallback.
- Added a new endpoint to support offline self-hosted Agent Builder usage.
- Fixed the alignment of 'no attachments' text in the UI.
- Fixed the render of polly shortcut with kbd styling like theme selector.
- Fixed the connect/disconnect button functionality for slack integrations.
- Added migrations for prompt environments in the backend.
- Fixed text overflow issue in DeleteConfirmationModal.
- Added inline editable display name feature on members table.
- Fixed tenant isolation in price map insertion.
- Fixed an issue to prevent the wrong tab from being highlighted on click in the frontend.
- Fixed vacuous connected state on the tool badge in Agent Builder.
- Added new authentication (Auth split ABAC + hub repos) capabilities.
