...
- Fixed path prefix for ABAC permission checks in self-hosted instances.
- Resolved DotDict handling in decrypt_passthrough_headers for Agent Builder.
- Added network allow denylist for improved security.
- Introduced access control UI for sandbox proxy.
- Supported Google IAP session refresh in self-hosted environments.
- Added JWT generation for auth-proxy in playground service.
- Enhanced frontend loading by reducing the number of preload chunks.
- Added localStorage caching for /info and /auth/v1/user responses to improve performance.
- Enabled OpenAI chat completion support.
- Added static TTL expiry fields to sandbox UI for better session management.
- Improved Playgound by making the apply button consistent in size.
- Added new access control features, including service key authentication for specific endpoints.
- Implemented Slack channel enablement for personal agents in the Agent Builder.
- Enhanced experimental evaluator functionality by raising SAQ timeouts.
- Improved design of the run detail dropdown.
- Resolved blank trace count dashboard when grouped by metadata key.
- Reordered inbox tabs to better prioritize attention-needed items.
- Added support for specifying environment variables from secret references in the host-backend.
- Fixed multiple user interface elements including checkbox replacements in various components.
- Updated tracing project column selector for better user experience.
- Added LangChain vendor extractor to messages feature.
- Fixed langgraph-api bump to address self-hosted 403 error.
