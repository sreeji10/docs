...
- Redesigned the run details view in threads to enhance user experience.
- Fixed the issue of popovers covering other content in the UI.
- Added Microsoft Outlook Calendar Tools in Agent Builder.
- Improved closing of agent chat meta popups with the Escape key.
- Made stats bar state dependent on local storage to improve usability.
- Disabled feedback comment filtering in the UI for self-hosted deployments.
- Enabled deep linking for thread details views for better navigation.
- Updated threading model to prevent the collapse of chat placeholders in Agent Builder.
- Improved streaming performance by skipping unnecessary mutations when search is disabled.
- Checked PostgreSQL and Redis connections on startup to ensure infrastructure readiness.
- Added single run filter type for more precise data handling.
- Supported multiple interrupts for long-running tasks in the Slack integration.
- Included the latest OpenAI GPT-5.3 model in model lists for enhanced capabilities.
- Introduced drag-to-resize feature for custom iframe output renderer in the frontend.
- Added a trace download button to enhance data export capabilities.
- Improved feedback layout with better alignment of feedback tags in annotation queues.
- Removed deprecated models from the unified model selector for a cleaner interface.
- Added tool calls as a default view in the frontend.
- Supported evaluator reuse in run rules to increase evaluation efficiency.
- Included Microsoft Teams Tools in Agent Builder for better collaboration features.
- Enhanced frontend with the ability to render audio and PDFs in foreign data tree and code components.
...
- Added rebranded primary colors to buttons under a feature flag in the frontend.
- Replaced dataset autocomplete with tag input for improved user input handling.
- Auto-hid and positioned the Models column based on data for a better UI experience.
- Fixed revalidation conflict in the frontend to improve usability.
- Fixed text overflow issue in workspace model configurations tooltips.
- Surfaced Models option in Group By popover with a feature flag for enhanced data organization.
- Supported loading ChatAnthropicVertex model configs for new model integration.
- Implemented auto-navigate to insights and global scroll for streamlined navigation.
- Fixed issues with the playground and evaluators provider selector not filtering disabled providers.
- Introduced the ability to create an agent from ZIP upload to enhance functionality.
- Added project filtering and search to the alerts table for better data management.
- Updated UX for messages mode with styling improvements for a better user experience.
- Implemented Raw Query Mode for Inline Filters to enhance filter capabilities.
- Fixed white-screen issue when loading eppo for improved stability.
- Fixed duplicated tool calls in messages view to improve performance.
- Properly deleted tools from the traces tool registry for better tool management.
- Added inbox feature with redesigned UI to the Agent Builder for better message handling.
- Fixed scroll bug in polly for smoother user interaction.
- Added security_contact to organizations for enhanced security communication.
- Dismissed "Add API Key to Get Started" dialog in the Agent Builder for better onboarding.
- Applied AND semantics for session filters in alerts for more accurate filtering.
- Persisted selected model in local storage for user convenience.
- Hid `__ls_model_name` and `__ls_runner` metadata behind an experiment_models_column feature flag for cleaner UI.
- Auto-generated thread titles for more organized discussion threads.
- Guarded secrets with granular RBAC permission for increased security.
- Persisted feedback configurations as JSONB objects for improved data handling.
- Introduced Outlook Email Tools to the Agent Builder for expanded integration.
- Added keyboard shortcuts to the inbox in the Agent Builder for enhanced user productivity.
...
- Fixed an issue where internal error details were leaked in 5xx responses, improving error message confidentiality.
- Improved the UI by moving the SaveViewButton to replace Display/Columns buttons and changed SaveForm from a popover to a modal.
- Added a model select dropdown to the template creation flow, enhancing the template customization process.
- Introduced a duplicate URL warning when creating MCP servers to prevent configuration conflicts.
- Added user context to agents and sub-agents for enhanced traceability and auditing.
- Added support for SerializedConstructor model configs from playground, expanding model configuration options.
- Improved scrolling layout in the run details view for a better navigation experience.
- Showed categorical feedback in experiment view config and hid unnecessary sort icons to streamline UI interaction.
- Added retry logic to the `gmail_apply_label` tool in the Agent Builder, improving tool reliability.
- Fixed cell alignment issues in playground and experiment views for a more consistent display.
- Added support for image uploads, offering new avenues for data input.
- Added an onboarding dialog to the general-purpose agent to guide new users.
- Fixed issue by adding a spinner to the loading triggers skeleton, enhancing loading feedback.
