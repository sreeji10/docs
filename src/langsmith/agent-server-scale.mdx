---
title: Scale agent deployment infrastructure
sidebarTitle: Scale agent deployments
description: Configure LangSmith Agent Server to handle high volumes of agent runs, threads, and API requests for your deployed agents
---

This page covers scaling **LangSmith Deployment**, the infrastructure that runs your **deployed agents**. It does not cover scaling observability or evaluation infrastructure.

<Tip>
To scale the infrastructure for **trace ingestion, evaluation, and the LangSmith UI**, see [scale self-hosted observability infrastructure](/langsmith/self-host-scale).
</Tip>
...
If an assistant requires synchronous blocking operations, set [`BG_JOB_ISOLATED_LOOPS`](/langsmith/env-var#bg_job_isolated_loops) to `True` to execute each run in a separate event loop.
...
##### Enable the use of queue workers <a name="enable-the-use-of-queue-workers"></a>
...
##### Support a number of jobs equal to expected throughput <a name="support-a-number-of-jobs-equal-to-expected-throughput"></a>
...
##### Configure autoscaling for bursty workloads <a name="configure-autoscaling-for-bursty-workloads-write"></a>
...
##### Configure autoscaling for bursty workloads <a name="configure-autoscaling-for-bursty-workloads-read"></a>
