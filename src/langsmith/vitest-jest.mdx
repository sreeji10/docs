...
* You should import `describe` and `test` from the @[`langsmith/jest`][langsmith/jest] or @[`langsmith/vitest`][langsmith/vitest] entrypoint.
...
You can think of each @[ls.test][ls.test] case as corresponding to a dataset example, and @[`ls.describe()`][ls.describe] as defining a LangSmith dataset. If you have LangSmith [tracing environment variables](/#3-set-up-your-environment) set when you run the test suite, the SDK does the following:
