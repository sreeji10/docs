...
You can add custom routes by providing your own app object and passing its path in `langgraph.json` (for example, a [`Starlette`](https://www.starlette.io/applications/) app in Python or a [`Hono`](https://hono.dev/) app in Javascript).
...
If you navigate to `localhost:2024/hello` in your browser (`2024` is the default development port), you should see the `/hello` endpoint returning a JSON response.
