...
<Note>
Regardless of the checkpointer backend, LangSmith always requires PostgreSQL for threads, runs, assistants, crons, and the [memory store](/oss/langgraph/persistence#memory-store). The checkpointer backend only controls where checkpoint data is stored.
</Note>
