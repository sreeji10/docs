...
Agent Server persists graph state using a checkpointer backend. By default, checkpoints are stored in Postgres alongside other server data. You can switch to MongoDB or provide a custom implementation.

<Note>
Regardless of the checkpointer backend, Postgres is always required for threads, runs, assistants, crons, and other application data. The checkpointer backend only controls where checkpoint data is stored.
</Note>
...
<Tabs>
  ...
  **External MongoDB** (production):
  ...
  <Tab title="Hybrid">
  Set `backend` to `"mongo"` in your `langgraph.json`, then set `LS_MONGODB_URI` as an environment variable through the control plane. Your MongoDB instance must be reachable from your data plane Kubernetes cluster.
  </Tab>
</Tabs>
