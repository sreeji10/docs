...
The @[`tracing_context`][tracing_context] context manager (Python) and @[`tracingEnabled`][tracingEnabled] option (TypeScript) allow you to override global tracing settings at runtime, without restructuring your code or changing environment variables.
...
When you use the @[`tracing_context`][tracing_context] context manager, it overrides the global tracing configuration for code executed within its scope. This means you can keep automatic tracing enabled globally while selectively controlling tracing behavior for specific function calls.
...
The @[`tracing_context`][tracing_context] context manager works with automatic tracing. You can keep `LANGSMITH_TRACING=true` set globally and use `tracing_context` to override settings for specific requests:
...
In TypeScript, you control tracing per-function using the @[`tracingEnabled`][tracingEnabled] parameter when calling @[`traceable()`][traceable]. This allows you to selectively enable or disable tracing at the function level.

A two-level system where tracing is controlled per-function:

1. **`tracingEnabled` parameter**: highest priority (pass to @[`traceable()`][traceable] config).
1. **Environment variables**: lowest priority (`LANGSMITH_TRACING`).
...
The @[`tracingEnabled`][tracingEnabled] option works seamlessly with automatic tracing. You can keep `LANGSMITH_TRACING=true` set globally and use `tracingEnabled` to override settings for specific functions.
