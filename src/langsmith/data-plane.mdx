...
PostgreSQL stores server resources (threads, runs, assistants, crons) and items saved in the [long-term memory store](/oss/langgraph/persistence#memory-store). It is also the default backend for [checkpoints](/oss/langgraph/persistence) (graph execution state). Checkpoints can optionally be stored in MongoDB instead — see [Configure checkpointer backend](/langsmith/configure-checkpointer). PostgreSQL is always required regardless of the checkpointer backend.
...
<Info>
Available for [Cloud](/langsmith/cloud) (with an externally-managed MongoDB instance), [Hybrid](/langsmith/hybrid), [Self-hosted](/langsmith/self-hosted), and [Standalone](/langsmith/deploy-standalone-server) deployments.
</Info>

MongoDB can be used as an alternative backend for checkpoint storage. When configured, MongoDB handles only checkpoint data — PostgreSQL remains required for all other server resources.
