...
When you call an LLM directly, outside of LangChain or a [supported integration](/langsmith/integrations), you need to provide specific metadata so that LangSmith can display token counts, calculate costs, and let you open the run in the [Playground](/langsmith/prompt-engineering-concepts#playground) with the correct provider and model.
...
The examples on this page use the `traceable` decorator/wrapper (the recommended approach for Python and JS/TS). The same requirements apply if you use the [RunTree](/langsmith/annotate-code#use-the-runtree-api) or [API](https://api.smith.langchain.com/redoc) directly.
</Note>
...
To send costs directly (for non-linear pricing), you can also include `input_cost`, `output_cost`, and `total_cost` fields. See [Cost tracking](/langsmith/cost-tracking) for details on configuring model pricing and viewing costs in the UI.
