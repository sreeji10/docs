...
If your data is already indexed and available for search (i.e., you have a function to execute a search), or if you're comfortable with [document loaders](/oss/integrations/document_loaders), [embeddings](/oss/integrations/embeddings), and [vector stores](/oss/integrations/vectorstores), feel free to skip to the next section on [retrieval and generation](/oss/langchain/rag#2-retrieval-and-generation).

</Note>

Indexing commonly works as follows:

1. **Load**: First we need to load our data. This is done with [Document Loaders](/oss/integrations/document_loaders).
...
We need to first load the blog post contents. We can use [DocumentLoaders](/oss/integrations/document_loaders) for this, which are objects that load in data from a source and return a list of @[Document] objects.
