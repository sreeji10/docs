...
| :--- | :--- | :--- |
| [OpenAI](/oss/integrations/providers/openai) | `langchain-openai` | `gpt-4.1` |
| [Anthropic](/oss/integrations/providers/anthropic) | `langchain-anthropic` | `claude-sonnet-4-6` |
| [Google](/oss/integrations/providers/google) | `langchain-google-genai` | `gemini-2.5-flash` |
| [AWS Bedrock](/oss/integrations/providers/aws) | `langchain-aws` | `us.anthropic.claude-sonnet-4-6-v1` |
| [Azure](/oss/integrations/providers/microsoft) | `langchain-openai` | `gpt-4.1` |
| [Ollama](/oss/integrations/providers/ollama) | `langchain-ollama` | `llama3` |

New model names work immediately — no LangChain update required — because provider packages pass model names directly to the provider's API.

For a complete list of providers, capabilities, and model names, see [Providers and models](/oss/concepts/providers-and-models) and the [integrations page](/oss/integrations/providers/overview).
...
4. The [Deep Agents CLI](/oss/deepagents/cli) filters the [interactive model switcher](/oss/deepagents/cli/providers#which-models-appear-in-the-switcher) to models whose profiles report `tool_calling` support and text I/O, and displays context window sizes and capability flags in the selector detail view.
