...
For a full list of provider packages, see the [integrations page](/oss/integrations/providers/overview).
...
<Tip>
When using @[`init_chat_model`] with the `provider:model` format, LangChain automatically resolves the provider and loads the correct integration package. You can also omit the provider prefix if the model name is unambiguous (e.g., `"gpt-4.1"` resolves to OpenAI).
</Tip>
...
For the full list of chat model integrations and their capabilities, see the [chat models integrations page](/oss/integrations/chat).
...
Many providers offer endpoints compatible with OpenAI's [Chat Completions API](https://platform.openai.com/docs/api-reference/chat). You can connect to these using [`ChatOpenAI`](/oss/integrations/chat/openai) with a custom `base_url`:
...
<CardGroup cols={2}>
    ...
    <Card title="All providers" icon="grid-2" href="/oss/integrations/providers/overview">
