...
[LangChain](/oss/langchain/overview) is an agent framework that provides abstractions like structured content blocks, the agent loop, and middleware.
...
While LangChain is built on top of [LangGraph](/oss/langgraph/overview), you don't need to know LangGraph to use LangChain.
...
[LangGraph](/oss/langgraph/overview) is a low-level orchestration framework and runtime for building, managing, and deploying long-running, stateful agents.
...
The [Deep Agents SDK](/oss/deepagents/overview) builds on top of LangGraph and adds planning capabilities, file systems for context management, the ability to spawn subagents, and more.
...
Use the [Deep Agents SDK](/oss/deepagents/overview) when:
