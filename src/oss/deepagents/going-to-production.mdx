...
<Accordion title="Example: syncing skills and memories with custom middleware">
  [Skill](/oss/deepagents/skills) scripts that the agent needs to execute must be uploaded into the sandbox before the agent runs. You may also want to sync [memories](/oss/deepagents/long-term-memory) so the agent can read and update them inside the container. Use [custom middleware](/oss/langchain/middleware/custom) with `before_agent` and `after_agent` hooks to move files across the sandbox boundary:
