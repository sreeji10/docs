...
<Tip>
    **Using the Deep Agents CLI?** The CLI has built-in sandbox support via the `--sandbox` flag. See [Use remote sandboxes](/oss/deepagents/cli/overview#use-remote-sandboxes) for CLI-specific setup, flags (`--sandbox-id`, `--sandbox-setup`), and examples.
</Tip>
...
If you provide a sandbox platform and want to contribute an integration, see [Contributing a sandbox integration](/oss/contributing/integrations-langchain).
...
Choose how sandbox lifecycles map to your application's resources. See [deployments](/oss/deepagents/deployments#sandboxes) for more on this decision.

| Scope | Sandbox ID stored on | Lifecycle | Best for |
|---|---|---|---|
| **Thread-scoped** (most common) | [Thread](/langsmith/use-threads) metadata | Created on first [run](/langsmith/runs), reused on follow-ups, cleaned up with thread | One-off coding tasks, data analysis, stateless work |
| **Assistant-scoped** | [Assistant](/langsmith/assistants) config | Created once, shared across all threads | Persistent dev environments, stateful workspaces |

**Thread-scoped** is the right default. Each conversation gets its own sandbox. The sandbox is created when the first run starts and reused for follow-up messages on the same thread. When the thread is cleaned up (or the sandbox TTL expires), the sandbox is destroyed.

**Assistant-scoped** sandboxes are shared across all threads for a given [assistant](/langsmith/assistants). The sandbox ID is stored on the assistant's configuration, so every conversation returns to the same environment. Files, installed packages, and cloned repositories persist across conversations. Use this when the agent maintains a long-running workspace.
