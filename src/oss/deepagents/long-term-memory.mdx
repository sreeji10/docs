...
Files stored via `StoreBackend` use the `FileData` schema:

```python
{
    "content": "line 1\nline 2\nline 3",         # String for text, Uint8Array for binary
...
# {'content': 'Hello\nWorld', 'mimeType': 'text/plain', 'created_at': '...', 'modified_at': '...'}
```
:::
...
<Note>
Legacy data (with `content` as a `string[]` of lines and no `mimeType`) is still readable. Use `migrateToFileDataV2()` to normalize legacy data, or set `BackendOptions.fileFormat: "v1"` during rolling deployments.
</Note>
