...
For more information, see the @[`create_deep_agent`] API reference.
:::
...
<Tip>For long-running agent tasks on unreliable networks, consider increasing `max_retries` to 10–15 and pairing it with a [checkpointer](/oss/langgraph/persistence) so that progress is preserved across failures.</Tip>
...
By default, Deep Agents have access to the following [middleware](/oss/langchain/middleware/overview):
...
LangChain exposes additional pre-built middleware that let you add-on various features, such as retries, fallbacks, or PII detection. See [Prebuilt middleware](/oss/langchain/middleware/built-in) for more.

:::python

The `deepagents` library also exposes @[create_summarization_tool_middleware], enabling agents to trigger summarization at opportune times—such as between tasks—instead of at fixed token intervals. For more detail, see [Summarization in the harness](/oss/deepagents/harness#summarization).

:::

:::js

The `deepagents` package also exposes @[createSummarizationMiddleware] for the same workflow. For more detail, see [Summarization in the harness](/oss/deepagents/harness#summarization).
...
:::js
    <Card title="Anthropic" href="/oss/integrations/middleware/anthropic" icon="/images/providers/anthropic-icon.svg" arrow>
        Prompt caching middleware for Claude models.
    </Card>
:::
