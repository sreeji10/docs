...
This guide demonstrates how to build a content writing agent from scratch using [Deep Agents](/oss/deepagents).

The agent you build will:

1. Load voice and workflow rules from `AGENTS.md` and skill folders
1. Delegate web research to a specialized subagent with `web_search`
1. Draft blog or social content following the loaded skill
1. Generate cover or social images with Gemini and save files under the project directory
...
- [Long-term memory](/oss/deepagents/long-term-memory) for TODO
- [Skills](/oss/deepagents/skills) for TODO
- [Subagents](/oss/deepagents/subagents) for TODO
- [Filesystem backends](/oss/deepagents/backends) for file read and write
- Custom [tools](/oss/langchain/tools) for search and image generation
...
<CodeGroup>
```bash pip wrap
pip install deepagents google-genai pillow pyyaml rich tavily-python langchain-core
```

```bash uv wrap
uv init
uv add deepagents google-genai pillow pyyaml rich tavily-python langchain-core
uv sync
```
</CodeGroup>
...
When you later create the agent and specify this file as part of the [memory](/oss/deepagents/long-term-memory) parameter, it gets loaded this into the system prompt so brand voice and research expectations apply to every run.
...
:::python
When creating the deep agent with @[create_deep_agent], pass memory paths, the skills directory, image tools, subagents from YAML, and a [FilesystemBackend](/oss/deepagents/backends) rooted at the example directory so paths like `./AGENTS.md` and `./skills/` resolve correctly.

<ContentBuilderCreateAgentPy />
:::

:::js
When creating the deep agent with @[createDeepAgent], pass memory paths, the skills directory, image tools, an inline subagent definition, and a [FilesystemBackend](/oss/deepagents/backends) rooted at the example directory so paths like `./AGENTS.md` and `./skills/` resolve correctly.
...
- Read [Subagents](/oss/deepagents/subagents), [Skills](/oss/deepagents/skills), and [Customization](/oss/deepagents/customization) for deeper configuration
