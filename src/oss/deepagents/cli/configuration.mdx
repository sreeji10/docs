...
Precedence at startup: **shell environment > project `.env` > global `.env`**. Values already set in the shell are never overwritten. Project-level values always take priority over the global file.

On `/reload`, project `.env` is re-read with override semantics (it can update shell vars set since startup), but the global file still loads as a fallback.
...
<AccordionGroup>
    ...
    ```bash
    export DEEPAGENTS_EXTRA_SKILLS_DIRS="~/shared-skills:/opt/team-skills"
    ```
    </Accordion>
</AccordionGroup>
...
<AccordionGroup>
    ...
    <Accordion title="Environment variable">
    ```bash
    export DEEPAGENTS_AUTO_UPDATE=1
    ```
    </Accordion>
</AccordionGroup>
