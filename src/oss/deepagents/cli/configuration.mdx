...
| File | Format | Purpose |
|------|--------|---------|
| `config.toml` | [TOML](https://toml.io/) | Model defaults, provider settings, constructor params, profile overrides, themes, update settings, MCP trust store |
...
This is useful when you need to scope credentials to the CLI without colliding with identically-named shell exports. On `/reload`, the CLI re-reads `.env` files and picks up prefixed values, so you can rotate keys without restarting.
...
<ResponseField name="api_key_env" type="string">
    Optionally override the environment variable name checked for credentials. Most chat model packages read from a default env var automatically — see the [Provider reference](/oss/deepagents/cli/providers#provider-reference) table for which variable each provider checks.
</ResponseField>
...
Some providers (e.g. `langchain-ollama`) don't bundle model profile data (see [Provider reference](/oss/deepagents/cli/providers#provider-reference) for full listing). When this is the case, the interactive `/model` switcher won't list models for that provider. You can fill in the gap by defining a `models` list in your config file for the provider:
...
By default, `load_skill_content` validates that a resolved skill file path stays inside one of the standard [skill directories](/oss/deepagents/skills#skill-locations). This prevents symlinks inside skill directories from reading arbitrary files outside those roots.
...
The CLI ships with many built-in themes. The selected theme is persisted under `[ui]`:
...
|----------|-------------|
| `DEEPAGENTS_CLI_AUTO_UPDATE` | Enable automatic CLI updates (`1`, `true`, or `yes`) |
| `DEEPAGENTS_CLI_DEBUG` | Enable verbose debug logging to a file |
| `DEEPAGENTS_CLI_DEBUG_FILE` | Path for the debug log file (default: `/tmp/deepagents_debug.log`) |
| `DEEPAGENTS_CLI_EXTRA_SKILLS_DIRS` | Colon-separated paths added to the [skill containment allowlist](#extra-allowed-directories) |
| `DEEPAGENTS_CLI_LANGSMITH_PROJECT` | Override the LangSmith project name for agent traces. See [Separate agent traces from app traces](/oss/deepagents/cli/overview#tracing-with-langsmith) |
| `DEEPAGENTS_CLI_NO_UPDATE_CHECK` | Disable automatic update checking when set |
| `DEEPAGENTS_CLI_SHELL_ALLOW_LIST` | Comma-separated shell commands to allow (or `recommended` / `all`) |
| `DEEPAGENTS_CLI_USER_ID` | Attach a user identifier to LangSmith trace metadata |
