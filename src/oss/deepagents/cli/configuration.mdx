...
By default, `load_skill_content` validates that a resolved skill file path stays inside one of the standard [skill directories](/oss/deepagents/data-locations#skills). This prevents symlinks inside skill directories from reading arbitrary files outside those roots.
...
| `DEEPAGENTS_CLI_LANGSMITH_PROJECT` | Override the LangSmith project name for agent traces. See [Tracing with LangSmith](/oss/deepagents/cli/overview#tracing-with-langsmith) |
