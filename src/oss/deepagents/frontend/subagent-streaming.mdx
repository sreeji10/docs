...
Deep agent subagent cards go beyond basic subagent streaming. Where the basic
pattern gives you a `Map` of subagent data, this pattern uses **filtered message
streams**, **per-message subagent lookup**, and a richer `SubagentStreamInterface`
to build detailed, collapsible cards for each specialist worker. The coordinator
agent spawns experts — a researcher, an analyst, a writer — and your UI shows
each one's progress, task description, streaming content, and timing in real
time.
...
## How it differs from basic subagent streaming

The basic [subagent streaming](/oss/deepagents/frontend/subagent-streaming) pattern exposes a
`subagents` map and lets you render status cards. Deep agent subagent cards add
three important capabilities:

| Feature | Basic | Deep |
| --- | --- | --- |
| Subagent tokens in main message stream | Yes | Filtered out with `filterSubagentMessages` |
| Link subagents to specific AI messages | No | Yes, via `getSubagentsByMessage(msg.id)` |
| Subagent interface | `SubagentData` | `SubagentStreamInterface` with richer metadata |
| Task description & type | Not available | Available via `toolCall.args` |
| Timing data | Not available | `startedAt` and `completedAt` timestamps |

This makes the deep pattern ideal for **complex multi-expert workflows** where
users need to understand *what* each subagent was asked to do, *how* it's
progressing, and *when* it finished.

## Setting up useStream

The critical option is `filterSubagentMessages: true`. This removes subagent
tokens from the main message stream so the coordinator's messages aren't
cluttered with interleaved subagent output.
...
<script lang="ts">
  ...
  const { messages, getSubagentsByMessage, submit } = useStream({
    apiUrl: AGENT_URL,
    assistantId: "deep_agent_subagent_cards",
    filterSubagentMessages: true,
  });
...
stream.submit(
  ...
  { streamSubgraphs: true, config: { recursion_limit: 100 } }
);
```

<Note>
The default recursion limit is 25. For deep agent workflows with multiple
nested subagents, increase this to avoid hitting the limit before all subagents
finish. A value of 100 is a safe starting point for most multi-expert setups.
</Note>

## The SubagentStreamInterface

Each subagent exposes a `SubagentStreamInterface` with richer metadata than the
basic `SubagentData`:
...
## The filterSubagentMessages option

When `filterSubagentMessages` is set to `true`, subagent tokens are removed
from the main `messages` array. Without this, every token produced by every
subagent would appear interleaved in the coordinator's message stream, making
it unreadable.

With filtering enabled:
- `stream.messages` contains only the coordinator's messages
- Subagent content is accessible through `stream.subagents` and
  `stream.getSubagentsByMessage`
- The UI stays clean: the coordinator's reasoning is separate from the
  specialists' work

<Info>
If you want to see raw, unfiltered output for debugging, set
`filterSubagentMessages: false` temporarily. This can be useful during
development to verify that subagent tokens are flowing correctly.
</Info>
...
- **Always set `filterSubagentMessages: true`** for production UIs. Unfiltered
  streams produce an unreadable interleaving of coordinator and subagent tokens.
