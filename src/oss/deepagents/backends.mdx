...
- You can pass either:
    ...
    - A factory `BackendFactory = Callable[[ToolRuntime], BackendProtocolV2]` (for backends that need runtime like `StateBackend` or `StoreBackend`).
...
from deepagents.backends.protocol import (
    BackendProtocolV2, WriteResult, EditResult,
    LsResult, ReadResult, ReadRawResult, GrepResult, GlobResult,
)

class S3Backend(BackendProtocolV2):
    ...
    def ls(self, path: str) -> LsResult:
        # List objects under _key(path); return LsResult(files=[...])
        ...

    def read(self, file_path: str, offset: int = 0, limit: int = 500) -> ReadResult:
        # Fetch object; return ReadResult(content=..., mimeType=...)
        # For binary files, return Uint8Array content
        ...

    def read_raw(self, file_path: str) -> ReadRawResult:
        # Return ReadRawResult(data=FileData)
        ...

    def grep(self, pattern: str, path: str | None = None, glob: str | None = None) -> GrepResult:
        # Search text files; skip binary; return GrepResult(matches=[...])
        ...

    def glob(self, pattern: str, path: str = "/") -> GlobResult:
        # Apply glob relative to path across keys; return GlobResult(files=[...])
        ...
...
- Map tool operations onto SQL:
  ...
  - `glob` filter in SQL or fetch then apply glob in Python → return `GlobResult`
...
class PolicyWrapper(BackendProtocolV2):
    def __init__(self, inner: BackendProtocolV2, deny_prefixes: list[str] | None = None):
        self.inner = inner
    ...
    def read_raw(self, file_path: str) -> ReadRawResult:
        return self.inner.read_raw(file_path)
    def grep(self, pattern: str, path: str | None = None, glob: str | None = None) -> GrepResult:
...
class PolicyWrapper implements BackendProtocolV2 {
  ...
  private deny(path: string): boolean {
    return this.denyPrefixes.some(p => path.startsWith(p));
  }

  ls(path: string) { return this.inner.ls(path); }
  read(filePath: string, offset?: number, limit?: number) { return this.inner.read(filePath, offset, limit); }
  readRaw(filePath: string) { return this.inner.readRaw(filePath); }
  grep(pattern: string, path?: string | null, glob?: string | null) { return this.inner.grep(pattern, path, glob); }
  glob(pattern: string, path?: string) { return this.inner.glob(pattern, path); }

  write(filePath: string, content: string): WriteResult {
    if (this.deny(filePath)) return { error: `Writes are not allowed under ${filePath}` };
    return this.inner.write(filePath, content);
  }

  edit(filePath: string, oldString: string, newString: string, replaceAll?: boolean): EditResult {
    if (this.deny(filePath)) return { error: `Edits are not allowed under ${filePath}` };
...
:::python
```python
result = backend.read("/workspace/screenshot.png")

if result.error:
    print(result.error)
elif isinstance(result.content, bytes):
    # Binary file — content is bytes, mime_type is set
    print(f"Binary file: {result.mime_type}")  # "image/png"
else:
    # Text file — content is str
    print(f"Text file: {result.mime_type}")  # "text/plain"
```
:::
...
<Accordion title="Legacy format">
  Older data may use a legacy format where `content` is a `string[]` (one entry per line) and `mimeType` is absent. The framework reads both formats transparently. Use `migrateToFileDataV2()` to normalize legacy data if needed.
</Accordion>

<Tip>
Use `BackendOptions.fileFormat` to control the write format. Set `fileFormat: "v1"` during rolling deployments where older readers need the legacy line-array format. The default is `"v2"`.
</Tip>
...
|----------|-------------|
| `createFileData(content, createdAt?, fileFormat?, mimeType?)` | Create a `FileData` object in the requested format |
| `isFileDataV1(data)` | Type guard for V1 format (content is `string[]`) |
| `migrateToFileDataV2(data, filePath)` | Convert V1 data to V2 (joins lines, infers MIME type) |
| `isTextMimeType(mimeType)` | Check if a MIME type is text-based |
| `getMimeType(filePath)` | Get MIME type from file extension |
...
:::python
```python
from deepagents.backends.utils import adapt_backend_protocol, adapt_sandbox_protocol

# Adapt a V1 backend to V2
v2_backend = adapt_backend_protocol(v1_backend)

# Adapt a V1 sandbox to V2
v2_sandbox = adapt_sandbox_protocol(v1_sandbox)
```
:::

<Note>
The framework auto-adapts V1 backends passed to `create_deep_agent()`. Manual adaptation is only needed when calling protocol methods directly.
</Note>

### FileData migration

Use `migrateToFileDataV2()` to normalize legacy V1 data (line arrays) to V2 format (single string + MIME type):

:::js
```typescript
import { migrateToFileDataV2, isFileDataV1 } from "deepagents";

if (isFileDataV1(data)) {
  const v2Data = migrateToFileDataV2(data, "/path/to/file.txt");
  // { content: "line1\nline2", mimeType: "text/plain", created_at: ..., modified_at: ... }
}
```
:::
...
:::python
```python
from deepagents.backends.utils import migrate_to_file_data_v2, is_file_data_v1

if is_file_data_v1(data):
    v2_data = migrate_to_file_data_v2(data, "/path/to/file.txt")
    # {"content": "line1\nline2", "mimeType": "text/plain", ...}
```
:::

For rolling deployments where older readers still expect V1 format, set `BackendOptions.fileFormat: "v1"` to continue writing in the legacy format.
