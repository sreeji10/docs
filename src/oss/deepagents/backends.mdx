...
const backend = (rt) => new StoreBackend(rt, {
  namespace: (ctx) => {
    const config = getConfig();
    return [config.configurable.threadId, "filesystem"];
  },
});
```
:::
...
<Note>
    When no namespace factory is provided, the legacy default uses the `assistant_id` from LangGraph config metadata. This means all users of the same [assistant](/langsmith/assistants) share the same storage. For multi-user [deployments](/oss/deepagents/deployments), always provide a namespace factory.
</Note>
