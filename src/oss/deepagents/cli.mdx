...
<Steps>
    <Step title="Set your API key" icon="key">
        ...
        <Note>
            ...
            For more information see [Additional installation and configuration options](/oss/deepagents/cli#additional-installation-and-configuration-options).
        </Note>
    </Step>
...
You can switch models during a session without restarting the CLI. The `/model` command supports both an interactive selector and direct switching.

### Interactive model selector

Run `/model` with no arguments to open a full-screen model selector:

```txt
> /model
```

The selector displays available models grouped by provider with credential indicators showing which providers are configured. Use the keyboard to navigate:

| Key | Action |
|-|-|
| `Up`/`Down`/`j`/`k`/`Tab` | Navigate models |
| Type | Filter/search models |
| `Enter` | Select model |
| `Esc` | Cancel |

You can also type a custom `provider:model` string directly into the search field and press `Enter` to use any model not listed.

### Direct model switching

Switch to a specific model by passing the model spec directly:
...
Bare model names are also supported with auto-detection:

```txt
> /model claude-sonnet-4-5
```

<Accordion title="Config file format">
    The CLI stores the default model in `~/.deepagents/config.toml`:
...
1. Optionally add additional scripts or other resources to the `test-skill` folder.
   For more information, see [Examples](/oss/deepagents/skills#examples).
