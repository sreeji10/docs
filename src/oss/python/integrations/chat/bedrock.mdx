...
You can filter to text using the [.text](https://reference.langchain.com/python/langchain-core/messages/ai/AIMessage) property on the output:
...
or LangChain's [standard types](/oss/langchain/messages#multimodal):
...
For detailed documentation on available parameters and features, refer to the [`ChatAnthropic` integration page](/oss/integrations/chat/anthropic).
