...
<Info>
    **AWS Bedrock and Google VertexAI**
    ...
    For Anthropic models on AWS Bedrock with the same API as `ChatAnthropic`, use [`ChatAnthropicBedrock`](/oss/integrations/chat/bedrock#chatanthropicbedrock) from `langchain-aws`.
</Info>
...
<Info>
    **Model Support**
    ...
    - Generally available: Claude Opus 4.6, Claude Sonnet 4.6, and Claude Opus 4.5
...
- **[Automatic caching](#automatic-caching)**: Pass `cache_control` at invocation time. The system automatically applies the cache breakpoint to the last cacheable block and moves it forward as conversations grow. Best for multi-turn conversations.
...
messages = [
    {
        "role": "system",
        "content": "You are a technology expert.",
    },
    {
        "role": "user",
...
Claude Sonnet 4.5 and Sonnet 4 support extending their context windows from 200k to 1 million tokens by using the `context-1m-2025-08-07` beta header, available for organizations in usage tier 4. Newer models (such as Claude Sonnet and Opus 4.6) support 1 million token context windows by default.
