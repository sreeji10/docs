...
description: "Integrate with the Astra DB vector store using LangChain Python."
---
...
The `AstraDBVectorStore` can be configured in many ways; see the [API Reference](https://reference.langchain.com/python/langchain-astradb/vectorstores/AstraDBVectorStore) for a full guide covering e.g. asynchronous initialization; non-Astra-DB databases; custom indexing allow-/deny-lists; manual hybrid-search control; and much more.
...
For a full list of the search modes available in `AstraDBVectorStore` check out the [API reference](https://reference.langchain.com/python/langchain-astradb/vectorstores/AstraDBVectorStore).
...
For detailed documentation of all `AstraDBVectorStore` features and configurations, consult the [API reference](https://reference.langchain.com/python/langchain-astradb/vectorstores/AstraDBVectorStore).
