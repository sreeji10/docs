---
title: "Google cloud Vertex AI reranker integration"
description: "Integrate with the Google cloud Vertex AI reranker document transformer using LangChain Python."
---
...
Let's inspect a couple of reranked documents. We observe that the retriever still returns the relevant LangChain type [documents](https://reference.langchain.com/python/langchain-core/documents/base/Document) but as part of the metadata field, we also receive the `relevance_score` from the Ranking API.
