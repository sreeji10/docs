...
See [Customization](/oss/javascript/deepagents/customization#middleware) for adding [custom middleware](/oss/javascript/langchain/middleware) and the [LangChain context engineering](/oss/javascript/langchain/context-engineering#system-prompt) guide for examples.

You do **not** need middleware when tools alone use context or `runtime.store`; tools receive `config.context` and `runtime.store` directly. Add middleware only when the system prompt itself must vary per request.
...
See [Harness](/oss/javascript/deepagents/harness) for built-in capabilities and [Customization](/oss/javascript/deepagents/customization#tools) for passing tools directly.
...
Runtime context is per-run configuration you pass when you invoke the agent. It is not automatically included in the model prompt; the model only sees it if a tool, middleware, or other logic reads it and adds it to messages or the system prompt. Use runtime context for user metadata (IDs, preferences, roles), API keys, database connections, feature flags, or other values your tools and harness need. Tools and subagents receive this context from the `config` argument:
...
Runtime context **propagates to all subagents**. When a subagent runs, it receives the same `config` (including `context`) as the parent. See [Subagents](/oss/javascript/deepagents/subagents#context-management) for per-subagent context (namespaced keys).
...
See [Long-term memory](/oss/javascript/deepagents/long-term-memory) for setup and use cases.
...
- [Harness](/oss/javascript/deepagents/harness) – Context management overview, offloading, summarization
- [Subagents](/oss/javascript/deepagents/subagents) – Context isolation, runtime context propagation
- [Long-term memory](/oss/javascript/deepagents/long-term-memory) – Cross-thread persistence
