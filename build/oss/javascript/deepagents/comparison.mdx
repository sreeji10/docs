...
- **Long-term memory**: Persist context across sessions and threads with the [Memory Store](/oss/javascript/deepagents/long-term-memory)
- **Sandbox-as-tool pattern**: Run individual operations in isolated sandboxes from [different providers](/oss/javascript/deepagents/sandboxes) while the agent runs outside, or run the full agent inside a sandbox
- **Virtual filesystem**: Use pluggable backends (in-memory, disk, durable stores, sandboxes) for context and checkpoint management
- **Production deployment**: Deploy via [LangSmith](/oss/javascript/langgraph/deploy) or self-host with the Agent Server
...
| Long-term memory | ✅ [Memory Store](/oss/javascript/deepagents/long-term-memory) | ❌ | ❌ |
...
| Production hosting | ✅ [LangGraph Platform](/oss/javascript/langgraph/deploy), self-hosted, or standalone | Build your own HTTP/WebSocket layer; run SDK in [containers](https://platform.anthropic.com/docs/en/agent-sdk/hosting) | ❌ |
