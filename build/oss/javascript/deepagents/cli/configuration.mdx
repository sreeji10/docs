...
By default, `load_skill_content` validates that a resolved skill file path stays inside one of the standard [skill directories](/oss/javascript/deepagents/skills#skill-locations). This prevents symlinks inside skill directories from reading arbitrary files outside those roots.
...
| `DEEPAGENTS_CLI_LANGSMITH_PROJECT` | Override the LangSmith project name for agent traces. See [Separate agent traces from app traces](/oss/javascript/deepagents/cli/overview#tracing-with-langsmith) |
