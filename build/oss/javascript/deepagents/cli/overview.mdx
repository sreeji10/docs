...
On first launch, the CLI automatically selects a model based on which API keys are available. After that, it remembers the most recently used model. You can also [set a persistent default](/oss/javascript/deepagents/cli/providers#setting-a-default-model) or pass `--model` at launch:
