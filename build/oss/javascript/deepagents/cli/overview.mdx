...
<Steps>
    <Step title="Set model credentials" icon="key">
        ...
        <Tip>
            Store keys in `~/.deepagents/.env` so they're available in every project without per-shell exports. See [environment variables](/oss/javascript/deepagents/cli/configuration#environment-variables).
        </Tip>
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/reload` - Re-read `.env` files, refresh runtime configuration (API keys, shell allow-list), and re-discover skills without restarting. Shell-exported variables always take precedence — use the [`DEEPAGENTS_CLI_` prefix](/oss/javascript/deepagents/cli/configuration#deepagents_cli_-prefix) to override them from `.env`. Conversation state is preserved
...
<Accordion title="Separate agent traces from app traces">
    When invoking the CLI programmatically from a LangChain application (e.g., as a subprocess in [non-interactive mode](/oss/javascript/deepagents/cli/non-interactive)), both your app and the CLI produce LangSmith traces. By default, these all land in the same project.
