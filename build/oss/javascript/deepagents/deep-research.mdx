...
- Multi-step planning with the [built-in planning tool](/oss/javascript/deepagents/harness#planning-capabilities)
...
Add the custom search tool. The `tavily_search` tool uses Tavily for URL discovery, then fetches full webpage content (converted to markdown) so the agent can analyze complete sources instead of summaries.
