...
For [deployed](/oss/javascript/deepagents/deployments) agents, always set a namespace factory to isolate data between users.
...
- **Multi-tenant**: Set a [namespace factory](/oss/javascript/deepagents/backends#namespace-factories) on `StoreBackend` to isolate data per user (for example, `namespace=lambda ctx: (ctx.runtime.context.user_id, "filesystem")`)
