...
When a skill is loaded, the CLI verifies that the resolved file path stays within one of these directories. Symlinks that resolve outside all skill roots are rejected. To allow symlink targets in additional directories, see [`[skills].extra_allowed_dirs`](/oss/javascript/deepagents/cli/configuration#extra-allowed-directories).
...
Each subagent is an `AGENTS.md` file with YAML frontmatter (`name`, `description`, optional `model`) and a markdown body for the system prompt. See [Custom subagents](/oss/javascript/deepagents/cli/overview#custom-subagents) for the full format reference.
