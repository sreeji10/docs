...
Note that [Command](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) only adds dynamic edges, while static edges will still execute. In other words, [Command](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) doesn't override static edges.
...
- Use [`Command`](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) when you need to **both** update the graph state **and** route to a different node. For example, when implementing [multi-agent handoffs](/oss/javascript/langchain/multi-agent/handoffs) where it's important to route to a different agent and pass some information to that agent.
...
[`Command`](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) is an important part of human-in-the-loop workflows: when using `interrupt()` to collect user input, [`Command`](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) is then used to supply the input and resume execution via `new Command({ resume: "User input" })`. Check out the [human-in-the-loop conceptual guide](/oss/javascript/langgraph/interrupts) for more information.
