...
<Steps>
    <Step title="Set your API key" icon="key">
        Select the model provider(s) you want to use and set your API key(s) as environment variables.
...
On first launch, the CLI automatically selects a model based on which API keys are available. After that, it remembers the most recently used model. You can also [set a persistent default](/oss/python/deepagents/cli/providers#setting-a-default-model) or pass `--model` at launch:
...
By default, shell execution is disabled in non-interactive mode. Use `--shell-allow-list` to allow specific commands:
...
For full details on switching models, setting a default, configuring custom providers, and using the `~/.deepagents/config.toml` config file, see [Custom model providers](/oss/python/deepagents/cli/providers).
