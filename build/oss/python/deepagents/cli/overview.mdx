...
* <Icon icon="arrows-minimize" size={16} /> **Context compaction** - summarize older conversation messages to reduce context window usage and costs during long sessions.
...
<Accordion title="Full list of built-in tools">
    ...
    | `compact_conversation` | Summarize older messages to reduce context window usage | Mixed<sup>2</sup> |
    ...
    <sup>2</sup>: The CLI automatically compacts the conversation in the background when token usage exceeds a model-aware threshold. The `compact_conversation` tool lets the agent (or you) trigger compaction on demand. When called as a tool, it requires user approval by default.
</Accordion>
...
For the full list of supported providers and configuration options, see [Custom model providers](/oss/python/deepagents/cli/providers).
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/tokens` - Display current context window token usage with a tree breakdown showing system prompt and conversation sizes
...
<Steps>
    ...
    <Step title="Run the CLI with a sandbox" icon="terminal">
        <Tabs>
            <Tab title="Daytona">
                ```bash
...
| Flag | Description |
|------|-------------|
| `--sandbox TYPE` | Sandbox provider to use: `modal`, `daytona`, or `runloop` (default: `none`) |
...
1. Optionally add additional scripts or other resources to the `test-skill` folder.
   For more information, see [Examples](/oss/python/deepagents/skills#example).
