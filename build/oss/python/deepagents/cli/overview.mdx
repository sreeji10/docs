...
* <Icon icon="plug" size={16} /> **[MCP tools](/oss/python/deepagents/cli/mcp-tools)** - load external tools from [Model Context Protocol](https://modelcontextprotocol.io/) servers.
* <Icon icon="chart-dots" size={16} /> **[Tracing](/oss/python/deepagents/cli/overview#tracing-with-langsmith)** - trace agent operations in LangSmith for observability and debugging.
...
<Steps>
    <Step title="Set model credentials" icon="key">
        ...
        <Tip>
            Store keys in `~/.deepagents/.env` so they're available in every project without per-shell exports. See [environment variables](/oss/python/deepagents/cli/configuration#environment-variables).
        </Tip>

        The CLI works with any LLM that supports tool calling — OpenAI, Anthropic, Google, Ollama, and many more. See [Providers](#providers) for setup details.
    </Step>
    ...
    <Step title="Enable tracing (optional)" icon="chart-dots">
        Add tracing keys to `~/.deepagents/.env` to see agent operations, tool calls, and decisions in [LangSmith](#tracing-with-langsmith):
...
The CLI is intentionally lightweight, shipping with OpenAI, Claude, and Gemini support out of the box. Each additional model provider is a separate dependency, so you only pull in what you need.
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/offload` (alias `/compact`) - Free up context window space by offloading messages to backend storage with a summary placeholder in the conversation. The agent can retrieve the full history from the offloaded file if needed
        ...
        - `/reload` - Re-read `.env` files, refresh runtime configuration (API keys, shell allow-list), and re-discover skills without restarting. Shell-exported variables always take precedence — use the [`DEEPAGENTS_CLI_` prefix](/oss/python/deepagents/cli/configuration#deepagents_cli_-prefix) to override them from `.env`. Conversation state is preserved
        - `/theme` - Open the interactive theme selector to switch color themes. Built-in themes are available plus any [user-defined themes](/oss/python/deepagents/cli/configuration#themes)
        ...
        - `/editor` - Open the current prompt in your external editor (`$VISUAL` / `$EDITOR`). See [External editor](/oss/python/deepagents/cli/configuration#external-editor)
...
For full details on switching models, setting a default, and adding custom model providers, see [Model providers](/oss/python/deepagents/cli/providers).
...
| `~/.deepagents/.env` | Global API keys and secrets. See [configuration](/oss/python/deepagents/cli/configuration#environment-variables) |
...
<AccordionGroup>
    <Accordion title="Add skills" icon="plus">
        ...
        1. Optionally add additional scripts or other resources to the `test-skill` folder. For more information, see [Examples](/oss/python/deepagents/skills#example).
...
Define custom [subagents](/oss/python/deepagents/subagents) as markdown files so the CLI agent can delegate specialized tasks to them. Each subagent lives in its own folder with an `AGENTS.md` file:
...
Project subagents override user subagents with the same name (see [precedence rules](/oss/python/deepagents/data-locations#subagents)).

The frontmatter requires `name` and `description` (same as the [`SubAgent` dictionary spec](/oss/python/deepagents/subagents#subagent-dictionary-based)). The markdown body becomes the subagent's `system_prompt`. In addition to the base spec, `AGENTS.md` files support an optional `model` frontmatter field that overrides the main agent's model for this subagent. Uses the `provider:model-name` format (e.g., `anthropic:claude-opus-4-6`, `openai:gpt-5.4`). Omit to inherit the main agent's model.
...
Extend the CLI with tools from external [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) servers. Place a `.mcp.json` at your project root and the CLI discovers it automatically. See the [MCP tools guide](/oss/python/deepagents/cli/mcp-tools) for configuration format, auto-discovery, and troubleshooting.
...
To override for a specific project, add the same keys to a `.env` in the project directory. See [environment variables](/oss/python/deepagents/cli/configuration#environment-variables) for the full loading order.
...
<Accordion title="Separate agent traces from app traces">
    When invoking the CLI programmatically from a LangChain application (e.g., as a subprocess in [non-interactive mode](/oss/python/deepagents/cli/non-interactive)), both your app and the CLI produce LangSmith traces. By default, these all land in the same project.
    ...
    You can also scope LangSmith credentials to the CLI using the [`DEEPAGENTS_CLI_` prefix](/oss/python/deepagents/cli/configuration#deepagents_cli_-prefix) (e.g., `DEEPAGENTS_CLI_LANGSMITH_API_KEY`).
</Accordion>
