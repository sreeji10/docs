...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/reload` - Refresh runtime configuration (API keys, `.env` changes, shell allow-list) and re-discover skills without restarting. Conversation state is preserved
...
<Accordion title="Separate agent traces from app traces">
    ...
    To send CLI traces to a dedicated project, set `DEEPAGENTS_LANGSMITH_PROJECT`:

    ```bash title="~/.deepagents/.env"
    DEEPAGENTS_LANGSMITH_PROJECT=my-deep-agent-execution
    ```
