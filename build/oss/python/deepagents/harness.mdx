...
| `read_file` | Read file contents with line numbers, supports offset/limit for large files. Natively supports binary files (images, PDFs, audio, video) via `ReadResult` with `mimeType`, returning them as multimodal content blocks. |
