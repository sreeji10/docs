...
<Tip>For long-running agent tasks on unreliable networks, consider increasing `max_retries` to 10–15 and pairing it with a [checkpointer](/oss/python/langgraph/concepts/checkpointers) so that progress is preserved across failures.</Tip>
