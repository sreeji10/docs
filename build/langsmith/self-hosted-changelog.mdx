...
- Fixed multiple security alerts in the frontend and backend to enhance overall security.
- Prevented XSS vulnerability via unescaped URL in the RichTextEditor.
- Improved security by restricting `--allow-run` to only the deno binary.
- Fixed playground issues to improve functionality in self-hosted environments.
...
- Fixed multiple security alerts in `smith-frontend` and `smith-polly` to enhance overall security.
- Fixed a code scanning alert related to code injection vulnerabilities.
- Restricted execution permissions in `smith-ace` to enhance security.
- Prevented XSS attacks via unescaped URLs in the RichTextEditor component of the frontend.
- Fixed issues with the playground in self-hosted environments.
