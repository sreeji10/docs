...
| | [Custom Slack bot](#set-up-a-custom-slack-bot) (Recommended) | [Default Slack bot](#set-up-the-default-slack-bot) |
|---|---|---|
...
| **DMs** | ✅ | ❌ |
| **Slack tools** | ✅ | ✅ |
| **Best for** | Direct back-and-forth communication from Slack. You can have multiple bots in a single channel, each tied to a different agent. | Starting a run every time a message is sent in a specific channel |
...
A custom Slack bot gives you full bidirectional communication between your agent and Slack. Unlike the default bot, a custom bot supports DMs, can be configured to only trigger when tagged, and gives your agent access to Slack tools for sending messages, replying in threads, and reading channel history.
...
<Steps>
  <Step title="Open the agent editor">
    In [Fleet](https://smith.langchain.com/agents), select your agent and click the <Icon icon="pencil"/> **Edit Agent** icon.
  </Step>
  <Step title="Add a Slack channel">
    In the **Channels** section, click **Slack**, then go to **LangSmith Bot** and click **Add Channel**.
  </Step>
  <Step title="Invite the app and paste the channel ID">
    In Slack, invite the default app to an existing channel. Once invited, copy the channel ID, paste it in the agent editor, and give it a name for you to recognize.
  </Step>
