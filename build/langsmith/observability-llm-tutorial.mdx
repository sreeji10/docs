...
Create an API key by navigating to the [LangSmith settings page](https://smith.langchain.com/settings). Then, set the following environment variables in your shell:
...
Wrap the OpenAI client with [`wrap_openai`](https://reference.langchain.com/python/langsmith/wrappers/_openai/wrap_openai) (Python) or [`wrapOpenAI`](https://reference.langchain.com/javascript/langsmith/wrappers/wrapOpenAI) (TypeScript):
...
import { v4 as uuidv4 } from "uuid"; // [!code highlight]
...
(async () => {
    const runId = uuidv4(); // [!code highlight]
    await supportBot("How many users can I have on the Starter plan?", { // [!code highlight]
        langsmithExtra: { runId }, // [!code highlight]
    }); // [!code highlight]
    const lsClient = new Client(); // [!code highlight]
    await lsClient.createFeedback(runId, "user-score", { score: 1.0 }); // [!code highlight]
})();
```
...
The following example traces both the retriever (with `run_type="retriever"`) and the main function (with a `metadata` attribute for the model name). Update `app.py` (or `app.ts`) from the previous step:
...
const retriever = traceable(  // [!code highlight]
    (query: string): string[] => docs,  // [!code highlight]
    { runType: "retriever" }  // [!code highlight]
);  // [!code highlight]
...
From **Monitoring** in the UI sidebar, select the tracing project you would like to monitor from the dropdown at the top of the page. This will open charts tracking key metrics for that tracing project over time, including trace count, latency, error rate, feedback scores, and costs. For more on available metrics and chart configuration, refer to [Dashboards](/langsmith/dashboards).
...
Because you have been logging the `llm` metadata attribute, you can group monitoring charts by that attribute to compare model performance over time. Click **Metadata** at the top of the Monitor tab to select a grouping attribute. The charts update to show results grouped by that attribute. For more on grouping and custom charts, refer to [Dashboards](/langsmith/dashboards).

### Drilldown

When a monitoring chart shows something unexpected, click a data point to jump directly to the filtered runs table for that time window. For more on searching and filtering runs, refer to [Filter traces](/langsmith/filter-traces-in-application).
