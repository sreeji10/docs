...
Each run (date range) in your export has its own [failure handling](#failure-scenarios) and [retry budget](#automation-retry-behavior). If a run fails after exhausting all retries, the entire export is marked as `FAILED`.
