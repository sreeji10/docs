...
Export time filters are based on `run.start_time`:
- The export `start_time` is **inclusive** — runs with `start_time` equal to the export start time are included.
- The export `end_time` is **exclusive** — runs with `start_time` equal to or after the export end time are excluded.
- This inclusive/exclusive behavior also applies to each partition's start and end times.

For example, an export with `start_time="2024-01-01T00:00:00Z"` and `end_time="2024-01-02T00:00:00Z"` will include all runs where `run.start_time >= 2024-01-01T00:00:00Z` and `run.start_time < 2024-01-02T00:00:00Z`.
...
- For spawned exports, the first time range exported is `start_time=(scheduled_export_start_time), end_time=(start_time + interval_hours)`.
    ...
    Since the end time is exclusive, consecutive exports do not overlap (see [Time filter behavior](#time-filter-behavior)).
...
Each bulk export is divided into multiple _runs_, where each run processes data for a [specific date partition](#partitioning-scheme) (typically organized by day). Runs are processed independently, which enables:
...
| **Storage/destination errors** | [Invalid credentials](#credentials-configuration), [missing bucket](#destinations---providing-a-s3-bucket), [permission issues](#permissions-required) | No | Fix destination configuration and create new export. |
...
2. **Review run errors**: Each run includes an [`errors` field](#error-handling) with detailed error messages keyed by retry attempt (e.g., `retry_0`, `retry_1`).
3. **Verify destination access**: Ensure your [destination bucket](#preparing-the-destination) still exists and [credentials](#credentials-configuration) are valid.
4. **Check run size**: If you see timeout errors, your date partitions may contain too much data. It may be helpful to [limit the exported fields](#limiting-exported-fields).
...
To monitor your export job, refer to [Monitoring the export job](#monitoring-the-export-job).
