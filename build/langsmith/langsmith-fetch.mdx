...
- **Project UUID**: Each project has a unique ID (UUID). You can find it in the project's URL or by hovering over <Icon icon="link"/> **ID** next to the project's name. This UUID can be used with the `--project-uuid` flag on CLI commands
