...
You can invoke Agent Builder agents from your applications using the [LangGraph SDK](/langsmith/reference) or the REST API. Agent Builder agents run on [Agent Server](/langsmith/agent-server), so you can use the same API methods as any other [LangSmith deployment](/langsmith/deployment).
